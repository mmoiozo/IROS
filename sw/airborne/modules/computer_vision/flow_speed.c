...
/**
 * @file modules/computer_vision/flow_speed.c
...
/* Static functions */
struct image_t *flow_speed_calc(struct image_t *img);     ///< The main optical flow calculation thread
...
void flow_speed_init(void)
{
  ...
  cv_add_to_device(&OPTICFLOW_CAMERA, flow_speed_calc);
 ...
 */
struct image_t *flow_speed_calc(struct image_t *img)
{
  // Copy the state
